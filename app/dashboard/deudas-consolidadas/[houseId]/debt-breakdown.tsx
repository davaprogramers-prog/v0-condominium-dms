...
export function DebtBreakdown({ debts, currencySymbol, onSelectionChange }: DebtBreakdownProps) {
  ...
  const handleToggle = (debtId: string) => {
    ...
    onSelectionChange(Array.from(newSelected), selectedTotal + (newSelected.has(debtId) ? debts.find((d) => d.id === debtId)?.amount || 0 : -(debts.find((d) => d.id === debtId)?.amount || 0)))
  }
  ...
  const handleSelectAll = (type: "fixed" | "variable" | "multa") => {
    ...
    Array.from(newSelected).forEach((id) => {
      const debt = debts.find((d) => d.id === id)
      newTotal += debt?.amount || 0
    })
    onSelectionChange(Array.from(newSelected), newTotal)
  }
