-- Update profiles table to add new roles
-- First, update the CHECK constraint by dropping and recreating it
-- This SQL should be executed manually in Supabase

ALTER TABLE profiles DROP CONSTRAINT IF EXISTS profiles_role_check;

-- Add the new constraint with all roles including 'conserje'
ALTER TABLE profiles
ADD CONSTRAINT profiles_role_check CHECK (role IN ('admin', 'owner', 'propietario', 'super_admin', 'conserje'));

-- If the table already has the constraint with different naming, you may need to use:
-- ALTER TABLE profiles ALTER COLUMN role TYPE TEXT;
-- Then re-add the constraint if it fails
...
CREATE POLICY "Users can view supply requests from their condo" ON supply_requests FOR SELECT USING (condo_id IN (SELECT condo_id FROM profiles WHERE id = auth.uid()));
CREATE POLICY "Concierge can create supply requests" ON supply_requests FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'conserje' AND condo_id = supply_requests.condo_id));
CREATE POLICY "Admins can manage supply requests" ON supply_requests FOR UPDATE USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin') AND condo_id = supply_requests.condo_id));

-- RLS Policies for concierge_worklogs
CREATE POLICY "Users can view worklogs from their condo" ON concierge_worklogs FOR SELECT USING (condo_id IN (SELECT condo_id FROM profiles WHERE id = auth.uid()));
CREATE POLICY "Concierge can create worklogs" ON concierge_worklogs FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'conserje' AND condo_id = concierge_worklogs.condo_id));
