...
SELECT 
  au.id,
  'super_admin',
  COALESCE(au.user_metadata->>'first_name', au.email::text),
  COALESCE(au.user_metadata->>'last_name', ''),
  au.email,
  NOW(),
  NOW()
FROM auth.users au
WHERE au.id NOT IN (SELECT id FROM public.profiles)
  AND (au.user_metadata->>'role' = 'super_admin' OR au.raw_user_meta_data->>'is_super_admin' = 'true')
...
FROM auth.users au
WHERE au.id NOT IN (SELECT id FROM public.profiles)
  AND au.user_metadata->>'role' = 'admin'
ON CONFLICT (id) DO NOTHING;
...
SELECT * FROM public.profiles WHERE updated_at >= NOW() - INTERVAL '1 minute';
