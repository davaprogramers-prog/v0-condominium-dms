import { put } from '@vercel/blob';
import fs from 'fs';
import path from 'path';

async function testBlobUpload() {
  try {
    console.log('[v0] Starting blob upload test...');
    
    // Create a small test image (1x1 pixel JPEG)
    const testImageBuffer = Buffer.from([
      
    ]);
    
    console.log('[v0] Test image buffer size:', testImageBuffer.length);
    
    // Create blob from buffer
    const blob = new Blob([testImageBuffer], { type: 'image/jpeg' });
    console.log('[v0] Blob created:', { size: blob.size, type: blob.type });
    
    // Generate filename
    const filename = `test-parcels/${Date.now()}-test.jpg`;
    console.log('[v0] Uploading filename:', filename);
    
    // Attempt upload
    const result = await put(filename, blob, {
      access: 'private',
      addRandomSuffix: false,
    });
    
    console.log('[v0] ✅ UPLOAD SUCCESS!');
    console.log('[v0] Result:', JSON.stringify(result, null, 2));
    console.log('[v0] URL:', result.url);
    console.log('[v0] Pathname:', result.pathname);
    
    return result;
  } catch (error) {
    console.error('[v0] ❌ UPLOAD FAILED!');
    console.error('[v0] Error type:', error.constructor.name);
    console.error('[v0] Error message:', error.message);
    if (error.stack) {
      console.error('[v0] Stack:', error.stack);
    }
    throw error;
  }
}

testBlobUpload().catch(err => {
  console.error('[v0] Test failed:', err);
  process.exit(1);
});
