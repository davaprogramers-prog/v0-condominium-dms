-- RLS Policies for super_admin and admin access
...
USING (
  condo_id IN (
    SELECT condo_id FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin')
  )
)
WITH CHECK (
  condo_id IN (
...
-- CONDO_EXPENSES
-- =============================================
DROP POLICY IF EXISTS "Admin can manage expenses" ON condo_expenses;
...
CREATE POLICY "Admin can manage expenses" ON condo_expenses
FOR ALL TO authenticated
USING (
  condo_id IN (
    SELECT condo_id FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin')
...
-- CONDO_INCOME
-- =============================================
DROP POLICY IF EXISTS "Admin can manage income" ON condo_income;
...
CREATE POLICY "Admin can manage income" ON condo_income
FOR ALL TO authenticated
USING (
  condo_id IN (
    SELECT condo_id FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin')
...
DROP POLICY IF EXISTS "Super admin full access expense_types" ON expense_types;
...
CREATE POLICY "Super admin full access expense_types" ON expense_types
FOR ALL TO authenticated
USING (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'super_admin')
)
...
DROP POLICY IF EXISTS "Super admin full access payment_proofs" ON payment_proofs;
...
CREATE POLICY "Super admin full access payment_proofs" ON payment_proofs
FOR ALL TO authenticated
USING (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'super_admin')
)
...
DROP POLICY IF EXISTS "Super admin full access common_areas" ON common_areas;
...
CREATE POLICY "Super admin full access common_areas" ON common_areas
FOR ALL TO authenticated
USING (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'super_admin')
)
...
-- =============================================
-- EXEMPTION_TYPES (corrected name)
-- =============================================
...
DROP POLICY IF EXISTS "Super admin full access exemption_types" ON exemption_types;
...
CREATE POLICY "Super admin full access exemption_types" ON exemption_types
FOR ALL TO authenticated
USING (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'super_admin')
)
...
-- BALANCE_ADJUSTMENTS
-- =============================================
DROP POLICY IF EXISTS "Admin can manage balance adjustments" ON balance_adjustments;
DROP POLICY IF EXISTS "Users can view balance adjustments" ON balance_adjustments;
DROP POLICY IF EXISTS "Super admin full access balance_adjustments" ON balance_adjustments;
...
CREATE POLICY "Admin can manage balance adjustments" ON balance_adjustments
FOR ALL TO authenticated
USING (
  condo_id IN (
    SELECT condo_id FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin')
...
CREATE POLICY "Super admin full access balance_adjustments" ON balance_adjustments
FOR ALL TO authenticated
USING (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'super_admin')
)
